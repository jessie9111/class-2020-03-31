...
```{r scene 2}
county %>% summarize(mean_povery = mean(poverty))

county %>% filter(name == "Fairfax County")
county %>% filter(name == "Santa Clara County")
county %>% filter(name == "Middlesex County" & state == "MA")

county %>% arrange(desc(less_than_hs))
```
...
```{r scene 3}
county %>% summarize(correlation = cor(poverty, less_than_hs))
```
...
```{r scene 4}
county %>%
  ggplot(aes(x = less_than_hs, y = poverty)) + geom_point() + geom_smooth(method = "lm")
```
...
```{r scene 5}
county %>%
  ggplot(aes(x = less_than_hs, y = poverty, color = region)) + geom_point() + transition_states(region)
```
...
This is modeling for explanation because we're looking at the relationship between two variables. On the other hand, modeling for prediction would be predicting another variable y based on the results of x. We are not trying to use one variable to predict the other.
...
county %>%
  ...
  summarize(average_poverty = mean(poverty))
```
...
county %>%
  mutate(good_education = ifelse(less_than_hs < 13, 1, 0)) %>%
  summarize(correlation = cor(good_education, poverty))
```
...
Want to see the difference between potential outcomes, but can't 
treatment - education, good education vs not good education
outcome - poverty level
potential outcomes - poverty level given education
unit - county
...
# poverty = 6.6 + 0.636(less_than_hs)
```
...
```{r}
county %>%
  rep_sample_n(size = nrow(county), reps = 1000, replace = TRUE) %>%
  nest()
```
